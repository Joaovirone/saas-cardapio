...
interface CarrinhoModalProps {
  ...
  onConfirmarPedido: (dadosPedido: string, enderecoFinal: string) => void; 
  carregando: boolean;
}
          ...
          {etapa === 'carrinho' && (
            itens.length === 0 ? (
              ...
              <View style={styles.content}>
                <FlatList data={itens} keyExtractor={(item) => item.produto.id} renderItem={renderItem} showsVerticalScrollIndicator={false} style={styles.itemsList} />
                <View style={styles.obsContainer}>
                  <Text style={styles.sectionTitle}>Alguma observação geral?</Text>
                  <TextInput style={styles.obsInput} placeholder="Ex: Tirar cebola, ponto da carne..." placeholderTextColor={COLORS.textSecondary} value={obsGerais} onChangeText={setObsGerais} multiline />
                </View>
          ...
          {etapa === 'checkout' && (
            <View style={styles.content}>
              <ScrollView showsVerticalScrollIndicator={false} style={{ maxHeight: 400 }}>
                ...
                {/* === A MÁGICA DO ENDEREÇO CONDICIONAL AQUI === */}
                {tipoEntrega === 'entrega' && (
                ...
                {formaPagamento === 'dinheiro' && (
                  <TextInput style={[styles.input, { marginTop: 12 }]} placeholder="Troco para quanto? (Ex: 100)" placeholderTextColor={COLORS.textSecondary} keyboardType="numeric" value={troco} onChangeText={setTroco} />
                )}
              </ScrollView>

              <View style={styles.footer}>
                <View style={styles.resumoRow}><Text style={styles.resumoLabel}>Subtotal</Text><Text style={styles.resumoValor}>R$ {total.toFixed(2).replace('.', ',')}</Text></View>
                <View style={styles.resumoRow}><Text style={styles.resumoLabel}>Taxa de {tipoEntrega === 'entrega' ? 'Entrega' : 'Retirada'}</Text><Text style={styles.resumoValor}>{taxaEntrega > 0 ? `R$ ${taxaEntrega.toFixed(2).replace('.', ',')}` : 'Grátis'}</Text></View>
                <View style={[styles.totalRow, { marginTop: 8, borderTopWidth: 1, borderColor: '#333', paddingTop: 12 }]}><Text style={styles.totalLabel}>Total Final</Text><Text style={styles.totalValue}>R$ {totalFinal.toFixed(2).replace('.', ',')}</Text></View>
...
const styles = StyleSheet.create({
  ...
  // Estilo do quadro de endereço salvo
  savedAddressBox: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#2a1a10', padding: 16, borderRadius: 12, borderWidth: 1, borderColor: COLORS.primary, gap: 12 },
  addressLabel: { color: COLORS.primary, fontSize: 12, fontWeight: 'bold', marginBottom: 4 },
