...
export function BottomNavBar({ abaAtiva, setAbaAtiva, quantidadeCarrinho, onAbrirCarrinho }: BottomNavBarProps) {
  return (
    <View style={styles.container}>
      ...
      >
        <Ionicons 
          name={abaAtiva === 'Cardapio' ? 'fast-food' : 'fast-food-outline'} 
          size={24} 
          color={abaAtiva === 'Cardapio' ? COLORS.primary : COLORS.textSecondary} 
        />
        <Text style={[styles.tabText, abaAtiva === 'Cardapio' && styles.tabTextActive]}>
          Cardápio
        </Text>
      </TouchableOpacity>

      {/* ABA 2: CARRINHO (Abre o Modal) */}
      <TouchableOpacity 
        style={styles.tab} 
      ...
      >
        <Ionicons 
          ...
          size={24} 
          color={abaAtiva === 'Perfil' ? COLORS.primary : COLORS.textSecondary} 
        />
...
const styles = StyleSheet.create({
  container: {
    ...
    alignItems: 'center',
    backgroundColor: COLORS.surface, // Fundo escuro dos cards
    paddingVertical: SPACING.sm,
    paddingBottom: Platform.OS === 'ios' ? 24 : SPACING.sm, // Espaço extra para a barrinha do iPhone
    borderTopWidth: 1,
    borderColor: '#333',
  },
  tab: {
